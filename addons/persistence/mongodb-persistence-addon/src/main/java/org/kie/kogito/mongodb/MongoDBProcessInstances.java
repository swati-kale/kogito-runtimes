...
public class MongoDBProcessInstances<T extends Model> implements MutableProcessInstances<T> {
    ...
    protected void updateStorage(String id,
                                 ProcessInstance<T> instance,
        ...
        if (isActive(instance)) {
            ...
            if (checkDuplicates) {
                ...
                if (clientSession != null) {
                    ...
                    if (piDoc==null) {
                        throw new MongoDBVersionConcurrencyException("The document with ID: "+id+" and Version: "+version+" is not found. It might have already updated.");
                    } 
                } else {
 ...
 @Override
    public void removeByVersion(String id, int version, Object... options) {
        ClientSession clientSession;
        if (options.length > 0 && options[0] instanceof ClientSession) {
        ...
        if (clientSession != null) {
            ProcessInstanceDocument piDoc = collection. findOneAndDelete(clientSession,Filters.and(Filters.eq(DOCUMENT_ID, id),Filters.eq("version", version)));
            if (piDoc==null) {
                throw new MongoDBVersionConcurrencyException("The document with ID: "+id+" and Version: "+version+" is not found. It might have already deleted.");
            }
        } else {
            collection.deleteOne(Filters.and(Filters.eq(DOCUMENT_ID, id),Filters.eq(VERSION, version)));
