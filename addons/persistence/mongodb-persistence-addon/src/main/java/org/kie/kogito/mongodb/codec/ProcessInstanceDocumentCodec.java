...
import static org.kie.kogito.mongodb.utils.DocumentUtils.PROCESS_INSTANCE;
...
public class ProcessInstanceDocumentCodec implements CollectibleCodec<ProcessInstanceDocument> {
    ...
    public void encode(BsonWriter writer, ProcessInstanceDocument piDoc, EncoderContext encoderContext) {
        ...
        doc.put(VERSION, piDoc.getVersions().entrySet().stream().map(e -> new Document().append(NAME, e.getKey()).append(VALUE, e.getValue())).collect(Collectors.toList()));
    ...
    public ProcessInstanceDocument decode(BsonReader reader, DecoderContext decoderContext) {
        ...
        piDoc.setVersions(document.getList(VERSION, Document.class).stream().collect(Collectors.toMap(d -> d.getString(NAME), d -> d.getInteger(VALUE))));
        return piDoc;
    }
}
