...
public class PersistableProcessInstances<T extends Model> implements MutableProcessInstances<T> {
    ...
    protected void updateStorage(String id, ProcessInstance<T> instance, boolean checkDuplicates) {
        ...
        if (isActive(instance)) {
            Document doc = marshaller.marshalProcessInstance(instance);
            if (checkDuplicates) {
                Document existing = collection.find(Filters.eq(DOCUMENT_ID, resolvedId)).first();
                if (existing != null) {
                    throw new ProcessInstanceDuplicatedException(id);
                } else {
                    doc.append(DOCUMENT_ID, new BsonString(resolvedId));
    ...
    private void reloadProcessInstance(ProcessInstance<T> instance, String resolvedId) {
        ((AbstractProcessInstance<?>) instance).internalRemoveProcessInstance(() -> {
            try {
                Document reloaded = collection.find(Filters.eq(DOCUMENT_ID, resolvedId)).first();
                if (reloaded != null) {
